...
import 'package:financas_app/services/chart_service.dart';
...
class _DynamicReportsScreenState extends State<DynamicReportsScreen> with SingleTickerProviderStateMixin {
  ...
  List<LineChartBarData> _cashFlowData = [];
  LineChartBarData _savingsData = LineChartBarData();
  ...
  Future<void> _loadChartData() async {
    // Carregar dados para o gráfico de pizza de despesas
    try {
      final expenseSections = await ChartService.getExpensesByCategoryChart();
      setState(() {
        _expenseSections = expenseSections;
        _isLoadingExpenses = false;
      });
    } catch (e) {
      print('Erro ao carregar dados de despesas: $e');
      setState(() {
        _isLoadingExpenses = false;
      });
    }
    
    // Carregar dados para o gráfico de linha de fluxo de caixa
    try {
      final cashFlowData = await ChartService.getCashFlowChart();
      setState(() {
        _cashFlowData = cashFlowData;
        _isLoadingCashFlow = false;
      });
    } catch (e) {
      print('Erro ao carregar dados de fluxo de caixa: $e');
      setState(() {
        _isLoadingCashFlow = false;
      });
    }
    
    // Carregar dados para o gráfico de linha de evolução das economias
    try {
      final savingsData = await ChartService.getSavingsEvolutionChart();
      setState(() {
        _savingsData = savingsData;
        _isLoadingSavings = false;
      });
    } catch (e) {
      print('Erro ao carregar dados de economias: $e');
      setState(() {
        _isLoadingSavings = false;
      });
    }
                                  ...
                                  titlesData: FlTitlesData(
                                    ...
                                    bottomTitles: AxisTitles(
                                      sideTitles: SideTitles(
                                        ...
                                        reservedSize: 30,
                                        interval: 1,
                                        getTitlesWidget: ChartService.getMonthTitleWidget,
                                      ),
                                    ),
                                    leftTitles: AxisTitles(
                                      sideTitles: SideTitles(
                                        showTitles: true,
                                        interval: 1000,
                                        getTitlesWidget: ChartService.getValueTitleWidget,
                                        reservedSize: 40,
                                      ),
                                    ),
                                  ),
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: _isLoadingExpenses
                        ? const Center(child: CircularProgressIndicator())
                        : _expenseSections.isEmpty
                            ? const Center(child: Text('Sem dados disponíveis'))
                            : PieChart(
                                PieChartData(
                                  sections: _expenseSections,
                                  sectionsSpace: 2,
                  ...
                  const SizedBox(height: 16),
                  // Legenda do gráfico
                  if (!_isLoadingExpenses && _expenseSections.isNotEmpty) ...[
